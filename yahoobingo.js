...
// var bingo = new Bingo(getCard());
// bingo.markRaffledNumber('N54');
// console.log(bingo.card.slots);
// assert(bingo.isWinner() === true);
...
// assert("#hasFullColumn returns false when it contains only numbers != 0", bingo.hasFullColumn() === false);

// bingo.setCard({slots: _.map(getCard().slots, function(row) { row[2] = 0; return row; })}); // map column 2 with 0
// assert("#hasFullColumn returns true if any column contains only zeros", bingo.hasFullColumn() === true);

// console.log(bingo.card.slots[2])

// bingo.card.slots.B[2] = 1;
// assert("#hasFullColumn returns false if any number in column is not a 0", bingo.hasFullColumn() === false);

// // assert("", bingo.hasFullRow() === false);
